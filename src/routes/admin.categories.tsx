...
const empty = { name_ar: "", sort_order: 0, image_url: "" };
...
function AdminCategories() {
  ...
  function edit(c: Category) {
    setEditing(c.id);
    setForm({ name_ar: c.name_ar, sort_order: c.sort_order, image_url: c.image_url ?? "" });
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3 rounded-2xl border border-border bg-card p-4">
        <div className="flex flex-wrap gap-2">
          <input placeholder="اسم القسم" value={form.name_ar} onChange={(e) => setForm({...form, name_ar: e.target.value})} className="flex-1 min-w-[200px] rounded-lg border border-border bg-background px-3 py-2 text-sm" />
          <input type="number" placeholder="الترتيب" value={form.sort_order} onChange={(e) => setForm({...form, sort_order: Number(e.target.value)})} className="w-24 rounded-lg border border-border bg-background px-3 py-2 text-sm" />
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={save} className="gold-gradient rounded-lg px-4 py-2 text-sm font-bold text-gold-foreground"><Plus className="h-4 w-4 inline"/> {editing ? "تحديث" : "إضافة"}</button>
          {editing && <button onClick={() => { setEditing(null); setForm({...empty}); }} className="rounded-lg border border-border px-4 py-2 text-sm">إلغاء</button>}
        </div>
      </div>
      <div className="rounded-2xl border border-border bg-card overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-surface text-xs uppercase text-muted-foreground">
            <tr><th className="px-4 py-2 text-start">الصورة</th><th className="px-4 py-2 text-start">الاسم</th><th className="px-4 py-2">الترتيب</th><th className="px-4 py-2">مفعّل</th><th></th></tr>
          </thead>
          <tbody>
            {data.map(c => (
              <tr key={c.id} className="border-t border-border">
                ...
                <td className="px-4 py-2 text-end space-x-2 space-x-reverse">
                  <button onClick={() => edit(c)} className="text-xs text-gold hover:underline">تعديل</button>
